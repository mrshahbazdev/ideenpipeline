...
<head>
    ...
    <title>Ideen-Management - {{ $currentTeam->name }} - {{ $tenant->subdomain }}</title>
...
<body class="bg-gray-50" x-data="{ mobileMenuOpen: false }">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-8">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p class="text-gray-600 mt-2">Strukturierte Übersicht aller Team-Ideen</p>
                </div>
                <div class="flex items-center space-x-3 text-white">
                    <a href="{{ route('tenant.ideas.index', ['tenantId' => $tenant->id]) }}" 
                       class="px-4 py-2 text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition">
                        <i class="fas fa-th-large mr-2"></i>Kartenansicht
                    </a>
                    <a href="{{ route('tenant.ideas.create', ['tenantId' => $tenant->id]) }}" 
                       class="px-6 py-3 bg-indigo-600 text-white rounded-lg font-semibold hover:bg-indigo-700 transition shadow-lg">
                        <i class="fas fa-plus mr-2 text-white"></i>Neue Idee
                    </a>
                </div>
            </div>
        </div>
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <p class="text-sm text-indigo-100 mb-1">
                            <i class="fas fa-users mr-1"></i>Aktives Team
                        </p>
                        <h3 class="text-2xl font-bold">{{ $currentTeam->name }}</h3>
                        <p class="text-sm text-indigo-100 mt-1">{{ $currentTeam->member_count }} Mitglieder</p>
                    </div>
                </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-6 gap-4 mb-8 text-white">
            <div class="bg-white rounded-lg shadow p-4">
                <p class="text-xs text-gray-600 mb-1">Ideen gesamt</p>
                <p class="text-2xl font-bold text-gray-900">{{ $stats['total_ideas'] }}</p>
            </div>
            <div class="bg-white rounded-lg shadow p-4">
                <p class="text-xs text-gray-600 mb-1">Offen</p>
                <p class="text-2xl font-bold text-yellow-600">{{ $stats['pending'] }}</p>
            </div>
            <div class="bg-white rounded-lg shadow p-4">
                <p class="text-xs text-gray-600 mb-1">Genehmigt</p>
                <p class="text-2xl font-bold text-green-600">{{ $stats['approved'] }}</p>
            </div>
            <div class="bg-white rounded-lg shadow p-4">
                <p class="text-xs text-gray-600 mb-1">In Umsetzung</p>
                <p class="text-2xl font-bold text-blue-600">{{ $stats['in_implementation'] }}</p>
            </div>
            <div class="bg-white rounded-lg shadow p-4">
                <p class="text-xs text-gray-600 mb-1">Durchschn. Schmerz</p>
                <p class="text-2xl font-bold text-orange-600">{{ $stats['avg_pain'] }}</p>
            </div>
            <div class="bg-white rounded-lg shadow p-4">
                <p class="text-xs text-gray-600 mb-1 text-white">Gesamtkosten</p>
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden text-white">
            <div class="overflow-x-auto text-white">
                <table class="w-full text-white">
                    <thead class="bg-gray-50 border-b border-gray-200 text-white">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider text-white">
                                Aktionen
                            </th>
                        </tr>
                    </thead>
                            ...
                            <tr class="table-row">
                                ...
                                <td class="px-4 py-4">
                                    <div class="max-w-xs">
                                        ...
                                        @else
                                            <span class="text-xs text-gray-400 italic">Keine Lösung angegeben</span>
                                        @endif
                                    </div>
                                </td>
                                ...
                                <td class="px-4 py-4 text-center">
                                    <span class="text-lg font-bold text-indigo-600">{{ number_format($idea->priority_1, 2, ',', '.') }}</span>
                                </td>
                                <td class="px-4 py-4 text-center">
                                    <span class="text-lg font-bold text-purple-600">{{ number_format($idea->priority_2, 2, ',', '.') }}</span>
                                </td>
                                <td class="px-4 py-4 text-center">
                                    @if($idea->in_implementation)
                                        <div class="flex flex-col items-center">
                                            ...
                                            @if($idea->implementation_date)
                                                <span class="text-xs text-gray-600">{{ $idea->implementation_date->format('d.m.Y') }}</span>
                                            @endif
                                        </div>
                                    @else
                                        <span class="text-gray-400">-</span>
                                ...
                                <td class="px-4 py-4 text-center">
                                    <button onclick="quickVote({{ $idea->id }}, this)" 
                                            class="inline-flex items-center px-3 py-2 rounded-lg transition {{ $idea->hasVoted($user) ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700' }} hover:bg-indigo-100">
                                ...
                                <td class="px-4 py-4">
                                    <div class="flex items-center justify-center space-x-3 text-white">
                                        <a href="{{ route('tenant.ideas.show', ['tenantId' => $tenant->id, 'idea' => $idea->id]) }}" class="text-indigo-600 hover:text-indigo-900" title="Details anzeigen">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        @if($idea->canEditBasic($user))
                                            <a href="{{ route('tenant.ideas.edit', ['tenantId' => $tenant->id, 'idea' => $idea->id]) }}" class="text-gray-600 hover:text-gray-900" title="Bearbeiten">
                                        ...
                                        @if($user->isAdmin())
                                            <form method="POST" action="{{ route('tenant.ideas.update-status', ['tenantId' => $tenant->id, 'idea' => $idea->id]) }}" class="inline">
                                                ...
                                                <button type="submit" class="text-green-600 hover:text-green-900" title="Genehmigen" onclick="return confirm('Diese Idee genehmigen?')">
                        ...
                        @empty
                            <tr>
                                <td colspan="11" class="px-4 py-12 text-center">
                                    ...
                                    <p class="text-gray-600 mb-2">Noch keine Ideen vorhanden</p>
                                    <a href="{{ route('tenant.ideas.create', ['tenantId' => $tenant->id]) }}" class="text-indigo-600 hover:text-indigo-800 font-semibold">
                                        <i class="fas fa-plus mr-1"></i>Erste Idee einreichen
                                    </a>
                                </td>
                            </tr>
                        @endforelse
        ...
        <div class="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-6">
            <h4 class="font-semibold text-blue-900 mb-3 text-sm flex items-center">
                <i class="fas fa-info-circle mr-2"></i>Formel für die Prioritätsberechnung
            </h4>
            <div class="grid md:grid-cols-2 gap-6 text-sm text-blue-800">
                <div class="bg-white rounded-lg p-4">
                    ...
                    <p class="text-xs text-gray-600">Niedriger ist besser - misst den Investitionsaufwand (Geld & Zeit)</p>
                    <div class="mt-2 text-xs">
                        <strong>Beispiel:</strong> (400 € / 100) + 3 Tage = 7.00
                    </div>
                </div>
                <div class="bg-white rounded-lg p-4">
                    <p class="font-bold text-purple-600 mb-2">Prio 2 = Prio 1 / Schmerz</p>
                    <p class="text-xs text-gray-600">Niedriger ist besser - ROI unter Berücksichtigung des Leidensdrucks</p>
                    <div class="mt-2 text-xs">
                        <strong>Beispiel:</strong> 7.00 / 8 = 0.88
                    </div>
                </div>
            </div>
            ...
            <div class="mt-4 bg-yellow-50 border border-yellow-200 rounded p-3">
                <p class="text-xs font-semibold text-yellow-900 mb-2">
                    <i class="fas fa-lightbulb mr-1"></i>Strategie für die Umsetzungsreihenfolge:
                </p>
                <ul class="text-xs text-yellow-800 space-y-1">
                    <li>• <strong>Hoher Schmerz + Niedrige Prio 2</strong> = Quick Win! (Großer Nutzen, günstig/schnell erledigt)</li>
                    <li>• <strong>Hoher Schmerz + Hohe Prio 2</strong> = Wichtig, aber teuer (sorgfältig planen)</li>
                    <li>• <strong>Niedriger Schmerz + Niedrige Prio 2</strong> = Geringe Priorität (Nice-to-have)</li>
                </ul>
            </div>
        </div>
    </div>
